 ...
 $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="nama" id="name" value="<?= $row['nama'] ?>" placeholder="input Nama" aria-describedby="emailHelp">
            </div>
            <div class="mb-3">
                <label class="form-label">Email</label>
                <input type="email" class="form-control" id="email" name="email" placeholder="Input Email" value="<?= $row['email'] ?>">
            </div>
...
$email = $_POST['email'];
/*Melakukan penyimpanan data*/
 $sql= "UPDATE pengguna SET nama='$nama', email='$email' WHERE id_pengguna ='$id_pengguna'";
 $conn->query($sql);
